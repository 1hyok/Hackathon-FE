package com.example.hackathon.core.data.repositoryimpl

import android.content.Context
import android.net.Uri
import com.example.hackathon.core.data.BuildConfig
...
import com.example.hackathon.core.network.dto.request.RegisterRequest
import com.example.hackathon.core.network.dto.request.UpdateCombinationRequest
import com.example.hackathon.core.network.dto.request.UpdateRegisterRequest
...
constructor(
    ...
    override suspend fun getCombinations(
        ...
        val paginatedCombinations =
            combinationsWithLikeStatus.subList(
    ...
    override suspend fun getCombinationById(id: String): Result<Combination> = try {
        delay(300)
    ...
    override suspend fun createCombination(
        ...
        if (BuildConfig.USE_MOCK_API) {
                ...
                if (imageUri != null) {
                    ...
                    if (base64String != null) {
                        ...
                        if (base64String.length > 1_000_000) {
                            android.util.Log.w(
                                "CombinationRepository",
                                "Image Base64 string is too large: ${base64String.length} bytes. Consider compressing the image."
                            )
                        }
                        listOf(base64String)
                    } else {
            ...
            if (BuildConfig.DEBUG) {
                android.util.Log.d(
                    "CombinationRepository",
                    "RegisterRequest: title=$title, categories=${listOf(
                        category.name
                    )}, ingredients count=${ingredients.size}, images count=${imageStrings.size}, description length=${description.length}"
                )
            }
            ...
            val userId =
                tokenManager.getUserIdFromToken()
                    ...
                    ?: throw IllegalStateException(
                        "User ID is not available. Please login again."
                    )
            ...
            val ingredientDtos =
                ingredients.map { ingredientString ->
                    ...
                    if (lastSpaceIndex > 0 &&
                        lastSpaceIndex < ingredientString.length - 1
                    ) {
                        IngredientDto(
    ...
    override suspend fun updateCombination(
        ...
        return try {
            if (BuildConfig.USE_MOCK_API) {
                ...
                delay(500)
                val allCombinations = DummyData.dummyCombinations + createdCombinations
                val existingCombination =
                    allCombinations.find { it.id == id }
                ...
                if (BuildConfig.DEBUG) {
                    android.util.Log.d(
                        "CombinationRepository",
                        "UpdateRegisterRequest: postId=$postId, title=$title, categories=${listOf(
                            category.name
                        )}, ingredients count=${ingredients.size}, images count=${imageStrings.size}"
                    )
                }
    ...
    override suspend fun deleteCombination(id: String): Result<Unit> = try {
        if (BuildConfig.USE_MOCK_API) {
            // Mock 모드: 로컬 데이터 삭제
            delay(300)
    ...
    override suspend fun getMyCombinations(): Result<List<Combination>> = try {
        delay(300)
        // TODO: 실제 API 호출
    ...
    override suspend fun getLikedCombinations(): Result<List<Combination>> = try {
        delay(300)
        val allCombinations = DummyData.dummyCombinations + createdCombinations
    ...
    override suspend fun likeCombination(id: String): Result<Unit> = try {
        delay(100)
        // 좋아요 토글 로직
    ...
    override suspend fun searchCombinations(query: String): Result<List<Combination>> = try {
        delay(300)
        val allCombinations = DummyData.dummyCombinations + createdCombinations
        val searchResults =
            allCombinations
    ...
    override suspend fun getRecipeDetail(id: Long): Result<RecipeDetail> = runCatching {
