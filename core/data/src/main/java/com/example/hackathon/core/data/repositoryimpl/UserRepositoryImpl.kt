...
constructor(
    ...
    override suspend fun getProfile(): Result<User> {
        return try {
            if (BuildConfig.USE_MOCK_API) {
                ...
                val userId =
                    tokenManager.getUserIdFromToken()?.toString()
                        ?: profile.id
                        ?: "unknown"
    ...
    override suspend fun updateProfile(nickname: String, profileImageUrl: String?): Result<User> {
        return try {
            if (BuildConfig.USE_MOCK_API) {
                ...
                // 실제 API 호출
                val request =
                    com.example.hackathon.core.network.dto.request.UpdateProfileRequest(
                        nickname = nickname,
                        profileImageUrl = profileImageUrl
                    )
