...
import com.example.hackathon.core.domain.repository.LoginResult
import com.example.hackathon.core.domain.repository.ReissueResult
import com.example.hackathon.core.domain.repository.SignupResult
import com.example.hackathon.core.model.DummyData
import com.example.hackathon.core.model.User
import com.example.hackathon.core.network.dto.request.LoginRequest
import com.example.hackathon.core.network.dto.request.LogoutRequest
import com.example.hackathon.core.network.dto.request.ReissueRequest
import com.example.hackathon.core.network.dto.request.SignupRequest
...
constructor(
    ...
    // 1. 회원가입
    override suspend fun signup(password: String, nickname: String): Result<SignupResult> =
        runCatching {
    ...
    // 2. 로그인
    override suspend fun login(nickname: String, password: String): Result<LoginResult> =
        runCatching {
            if (BuildConfig.USE_MOCK_API) {
                // Mock 모드: 더미 데이터 반환
                delay(800) // 네트워크 시뮬레이션
                ...
                // loginResponse.user 는 다른 모듈(core:network)의 nullable public property 라
                // smart cast 불가 → 지역변수로 스냅샷 후 분기.
                val responseUser = loginResponse.user
                val user =
                    if (responseUser != null) {
                        responseUser.toEntity()
                    } else {
                        // 사용자 정보를 별도로 조회
    ...
    // 3. 토큰 재발급
    override suspend fun reissue(): Result<ReissueResult> = runCatching {
        val refreshToken =
