...
import com.example.hackathon.core.data.mapper.toDomain
...
constructor(private val recipeService: RecipeService) :
    RecipeRepository {
    // 1. 전체 랭킹 조회(홈화면)
    override suspend fun getRecipeRanking(page: Int?): Result<RecipeRanking> = runCatching {
    ...
    // 3. 검색/해시태그검색
    override suspend fun searchRecipes(keyword: String): Result<RecipeRanking> = runCatching {
    ...
    // 4. 레시피 상세조회
    override suspend fun getRecipeDetail(postId: Long): Result<RecipeDetail> = runCatching {
    ...
    // 5. 좋아요
    override suspend fun likeRecipe(id: Long): Result<Unit> = try {
        if (BuildConfig.USE_MOCK_API) {
            ...
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    Result.failure(
                        Exception("서버 오류: ${response.code()} ${response.message()}")
                    )
                }
    ...
    // 6. 좋아요 취소
    override suspend fun unlikeRecipe(postId: Long): Result<Unit> = try {
        if (BuildConfig.USE_MOCK_API) {
            // Mock 모드: 네트워크 시뮬레이션
            delay(300)
            Result.success(Unit)
