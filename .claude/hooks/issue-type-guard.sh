...
if [ "$actual_type" = "null" ]; then
    jq -nc --arg url "$url" --arg n "$issue_num" \
      '{hookSpecificOutput: {hookEventName: "PostToolUse", additionalContext: ("Issue \($url) 의 issueType 누락. Task/Bug/Feature 중 명시 + GraphQL updateIssue 즉시 호출 필요. 예: gh api graphql -f query=\"mutation { updateIssue(input: { id: <issue-node-id>, issueTypeId: <type-node-id> }) { issue { number issueType { name } } } }\". Type 노드 ID 는 repo 마다 다름 — gh api graphql 로 repository(owner,name) 의 issueTypes 노드를 조회해 사용.")}}'
    exit 0
fi
