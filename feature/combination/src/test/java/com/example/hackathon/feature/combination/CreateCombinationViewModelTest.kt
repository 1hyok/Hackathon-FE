package com.example.hackathon.feature.combination
...
class CreateCombinationViewModelTest {
    ...
    @Test
    fun `제목과 설명 입력 시 상태가 업데이트된다`() = runTest {
        // Given
        val title = "서브웨이 꿀조합"
    ...
    @Test
    fun `태그 추가 시 태그 리스트에 추가된다`() = runTest {
        // Given
        val tag = "#서브웨이"
    ...
    @Test
    fun `조합 등록 성공 시 로딩 상태가 변경된다`() = runTest {
        // Given
        val mockCombination =
            Combination(
                id = "1",
    ...
    @Test
    fun `제목이 비어있으면 에러 메시지가 표시된다`() = runTest {
        // Given
    ...
    @Test
    fun `재료가 비어있으면 에러 메시지가 표시된다`() = runTest {
        // Given
    ...
    @Test
    fun `조합 등록 실패 시 에러 상태가 표시된다`() = runTest {
        // Given
        viewModel.updateTitle("제목")
        viewModel.updateDescription("설명")
        viewModel.updateIngredientName(0, "재료1")
