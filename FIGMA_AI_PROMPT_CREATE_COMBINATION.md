...
1. 상단 바 (Top Bar)
   ...
   - 제목: "조합 등록" (Head2_bold, 22px, White 텍스트)
   - 뒤로가기 아이콘 (왼쪽, White 아이콘)
...
3. 입력 필드들 (OutlinedTextField 스타일)
   ...
   - 공개 여부: 토글 또는 선택 버튼
     - "공개" / "비공개" 두 개의 버튼
     - 선택된 버튼: Primary 색상 배경 (#E10818), White 텍스트
     - 선택되지 않은 버튼: White 배경, Primary 테두리, Primary 텍스트
   - 재료 입력: 멀티라인 텍스트 영역
     - 플레이스홀더: "재료"
     - 여러 줄 입력 가능
   - 만드는 방법 입력: 멀티라인 텍스트 영역
     - 플레이스홀더: "만드는 방법"
     - 여러 줄 입력 가능
   - 설명 입력: 멀티라인 텍스트 영역
     - 플레이스홀더: "조합에 대한 설명을 입력하세요"
